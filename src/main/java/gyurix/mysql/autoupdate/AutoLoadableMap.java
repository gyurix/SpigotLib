...
public interface AutoLoadableMap<K, V> extends AutoLoadable, Map<K, V> {
  ...
  default V processPut(K key, V value) {
    System.out.println("AutoLoadableMap - put - " + key + " - " + value);
    V out = putNow(key, value);
    if (out != value) {
      if (value instanceof AutoUpdatable)
        ((AutoUpdatable) value).setupForInsertion(getKey() + "." + key, getDb());
      if (out == null)
  ...
  default V processRemove(Object key) {
